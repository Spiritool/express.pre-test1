...
class Model_pendidikan {
    ...
    static async getAll() {
        return new Promise((resolve, reject) => {
            connection.query(`SELECT pendidikan.*, mahasiswa.nama_depan AS mahasiswa_nama_depan 
            FROM pendidikan JOIN mahasiswa ON pendidikan.id_mahasiswa = mahasiswa.id_mahasiswa`, (err, rows) => {
                if (err) {
                    reject(err);
                } else {
                    resolve(rows);
